...
public class FileScanner implements Runnable {
	...
	@Inject
	private FileScanner(PmsCore pmsCore) throws InitialisationException {
		...
		try {
			...
			throw new InitialisationException(
					"Both components FullDataCollector and MediaLibraryStorage have to be configured before this method can be called",
					e);
		}
	}
	...
	public void scan(DOManagedFile mFolder) {
		...
		if (folderToScan.isDirectory()) {
			pmsCore.getFrame().setStatusLine(
					String.format(
							Messages.getString("ML.FileScanner.ScanFolder"),
							folderToScan.getAbsoluteFile()));
			...
			if (childPaths != null) {
				for (int i = 0; i < childPaths.length; i++) {
					...
					DOManagedFile tmpFile = new DOManagedFile(
							mFolder.isWatchEnabled(), currentFile.toString(),
							mFolder.isVideoEnabled(), mFolder.isAudioEnabled(),
							mFolder.isPicturesEnabled(),
							mFolder.isSubFoldersEnabled(),
							mFolder.isFileImportEnabled(),
							mFolder.getFileImportTemplate());
				...
				log.debug("No children found for folder "
						+ folderToScan.getAbsolutePath());
			}
		}
	}
	...
	public void pause() throws ScanStateException {
		...
		if (scanState != ScanState.RUNNING) {
			throw new ScanStateException(
					ScanState.RUNNING,
					scanState,
					"The pause() method can only be called when the ScanState==RUNNING. current state="
							+ scanState);
		}
	...
	public void unPause() throws ScanStateException {
		...
		if (scanState != ScanState.PAUSED) {
			throw new ScanStateException(ScanState.PAUSED, scanState,
					"The pause() method can only be called when the ScanState==PAUSED. current state="
							+ scanState);
		}
	...
	public void run() {
		...
		while ((mf = dequeueManagedFile()) != null) {
			...
			if (f.isFile()) {
				...
				if (scanState == ScanState.PAUSING) {
					try {
						...
						pmsCore.getFrame().setStatusLine("Scan paused");
						changeScanState(ScanState.PAUSED);

						synchronized (scanThreadPause) {
							while (scanState == ScanState.PAUSING
									|| scanState == ScanState.PAUSED) {
								scanThreadPause.wait();
							}
						}

						pmsCore.getFrame().setStatusLine("Restarted scan");
				...
				if (dateLastUpdate.before(comp.getTime())) {
					...
					try {
						fileInfo = dataCollector.get(new DOManagedFile(mf
								.isWatchEnabled(), mf.getPath().toString(), mf
								.isVideoEnabled(), mf.isAudioEnabled(), mf
								.isPicturesEnabled(), mf.isSubFoldersEnabled(),
								mf.isFileImportEnabled(), mf
										.getFileImportTemplate()));
					} catch (Throwable t) {
						log.error("Failed to collect info for " + mf.getPath(),
								t);
						continue;
					}
					...
					if (fileInfo != null) {
						if (mediaLibraryStorage.getFileInfoLastUpdated(
								f.getAbsolutePath()).equals(new Date(0))) {
			...
			if (scanState == ScanState.STOPPING) {
				if (log.isInfoEnabled()) {
					log.info("Scan stopped after stopping request");
				}
				break;
			}
		}

		pmsCore.getFrame().setStatusLine(String.format(Messages.getString("ML.Messages.ScanFinished"),
				String.valueOf(nbFilesAdded)));

		if (log.isInfoEnabled()) {
			log.info("Finished scanning " + nbFilesAdded + " files");
		}

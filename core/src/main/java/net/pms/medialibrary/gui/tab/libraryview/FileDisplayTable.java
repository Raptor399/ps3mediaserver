...
public class FileDisplayTable extends JPanel {
	...
	private final IMediaLibraryStorage mediaLibraryStorage;
	...
	public interface Factory {
		public FileDisplayTable create(FileType fileType);
	}

	@AssistedInject
	public FileDisplayTable(PmsCore pmsCore, IMediaLibraryStorage mediaLibraryStorage, @Assisted FileType fileType) {
		super(new BorderLayout());
		this.mediaLibraryStorage = mediaLibraryStorage;
	...
	private void updateSelectedFiles(DOFileImportTemplate template) {
		...
		IProgress progressReporter = new IProgress() {
			...
			public void workComplete() {
				repaint();
				pmsCore.getFrame().setStatusLine(String.format(Messages.getString("ML.Messages.UpdateFinished"), nbFilesToUpdate));
			}
	...
	private void deleteSelectedFiles() {
		...
		int dialogResponse = JOptionPane.showOptionDialog((JFrame) (SwingUtilities.getWindowAncestor((Component) pmsCore.getFrame())),
	...
	private Dimension getMultiEditSize() {
		...
		int height = 430;
		try {
			width = Integer.parseInt(mediaLibraryStorage.getMetaDataValue("FileEditDialog_MultiEdit_Width"));
			height = Integer.parseInt(mediaLibraryStorage.getMetaDataValue("FileEditDialog_MultiEdit_Height"));
		} catch (NumberFormatException ex) { }
		return new Dimension(width, height);
	}
	
	private void setMultiEditSize(Dimension size) {
		mediaLibraryStorage.setMetaDataValue("FileEditDialog_MultiEdit_Width", String.valueOf(size.width));
		mediaLibraryStorage.setMetaDataValue("FileEditDialog_MultiEdit_Height", String.valueOf(size.height));
	}
	...
	private Dimension getSingleEditSize() {
		...
		int height = 420;
		try {
			width = Integer.parseInt(mediaLibraryStorage.getMetaDataValue("FileEditDialog_SingleEdit_Width"));
			height = Integer.parseInt(mediaLibraryStorage.getMetaDataValue("FileEditDialog_SingleEdit_Height"));
		} catch (NumberFormatException ex) { }
		return new Dimension(width, height);
	}
	
	private void setSingleEditSize(Dimension size) {
		mediaLibraryStorage.setMetaDataValue("FileEditDialog_SingleEdit_Width", String.valueOf(size.width));
		mediaLibraryStorage.setMetaDataValue("FileEditDialog_SingleEdit_Height", String.valueOf(size.height));
	}

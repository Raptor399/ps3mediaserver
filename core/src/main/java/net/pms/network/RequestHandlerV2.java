...
public class RequestHandlerV2 extends SimpleChannelUpstreamHandler {
 ...
 Pattern.compile("timeseekrange\\.dlna\\.org\\W*npt\\W*=\\W*([\\d\\.:]+)?\\-?([\\d\\.:]+)?",
	...
	private final RequestV2.Factory requestV2Factory;
	...
	public interface Factory {
		public RequestHandlerV2 create(ChannelGroup group);
	}

	@AssistedInject
	public RequestHandlerV2(PmsCore pmsCore, PmsConfiguration configuration, RequestV2.Factory requestV2Factory,
			@Assisted ChannelGroup group) {
		this.pmsCore = pmsCore;
		this.configuration = configuration;
		this.requestV2Factory = requestV2Factory;
		this.group = group;
	...
	public void messageReceived(ChannelHandlerContext ctx, MessageEvent e)
		...
		if (HttpMethod.GET.equals(nettyRequest.getMethod())) {
			request = requestV2Factory.create("GET", nettyRequest.getUri().substring(1));
		} else if (HttpMethod.POST.equals(nettyRequest.getMethod())) {
			request = requestV2Factory.create("POST", nettyRequest.getUri().substring(1));
		} else if (HttpMethod.HEAD.equals(nettyRequest.getMethod())) {
			request = requestV2Factory.create("HEAD", nettyRequest.getUri().substring(1));
		} else {
			request = requestV2Factory.create(nettyRequest.getMethod().getName(), nettyRequest.getUri().substring(1));
		}

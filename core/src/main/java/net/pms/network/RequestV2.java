...
public class RequestV2 extends HTTPResource {
	...
	public interface Factory {
		public RequestV2 create(@Assisted("method") String method,
	 ...
	 * 
	 * @param method
	 *            The {@link String} that defines the HTTP method to be used.
	 * @param argument
	 *            The {@link String} containing instructions for PMS. It
	 *            contains a command, a unique resource id and a resource name,
	...
	public ChannelFuture answer(
		...
		if ((method.equals("GET") || method.equals("HEAD")) && argument.startsWith("console/")) {
			...
			List<DLNAResource> files = pmsCore.getRootFolder(mediaRenderer).getDLNAResources(id, false, 0, 0, mediaRenderer);
			...
			if (files.size() == 1) {
				...
				if (fileName.startsWith("thumbnail0000")) {
					...
					if (subtitleHttpHeader != null && !"".equals(subtitleHttpHeader)) {
						...
						if (subs != null && !subs.isEmpty()) {
							...
							if (isNotBlank(subExtension)) {
								subtitleUrl = "http://" + pmsCore.getServer().getHost()
										+ ':' + pmsCore.getServer().getPort() + "/get/"
					...
					if (inputStream == null) {
						...
						pmsCore.getFrame().setStatusLine("Serving " + name);
			...
			if (argument.equals("description/fetch")) {
				...
				s = s.replace("[uuid]", pmsCore.getUuid());//.substring(0, pmsCore.usn().length()-2));
				String profileName = configuration.getProfileName();
				if (pmsCore.getServer().getHost() != null) {
					s = s.replace("[host]", pmsCore.getServer().getHost());
					s = s.replace("[port]", "" + pmsCore.getServer().getPort());
				}
				if (xbox) {
			...
			if (soapaction.indexOf("ContentDirectory:1#GetSystemUpdateID") > -1) {
				...
				if (xbox && configuration.getUseCache() && pmsCore.getLibrary() != null && containerID != null) {
					if (containerID.equals("7") && pmsCore.getLibrary().getAlbumFolder() != null) {
						objectID = pmsCore.getLibrary().getAlbumFolder().getResourceId();
					} else if (containerID.equals("6") && pmsCore.getLibrary().getArtistFolder() != null) {
						objectID = pmsCore.getLibrary().getArtistFolder().getResourceId();
					} else if (containerID.equals("5") && pmsCore.getLibrary().getGenreFolder() != null) {
						objectID = pmsCore.getLibrary().getGenreFolder().getResourceId();
					} else if (containerID.equals("F") && pmsCore.getLibrary().getPlaylistFolder() != null) {
						objectID = pmsCore.getLibrary().getPlaylistFolder().getResourceId();
					} else if (containerID.equals("4") && pmsCore.getLibrary().getAllFolder() != null) {
						objectID = pmsCore.getLibrary().getAllFolder().getResourceId();
				...
				List<DLNAResource> files = pmsCore.getRootFolder(mediaRenderer).getDLNAResources(objectID, browseFlag != null && browseFlag.equals("BrowseDirectChildren"), startingIndex, requestCount, mediaRenderer);
			...
			output.setHeader("SID", pmsCore.getUuid());
			...
			try {
				...
				out.write(("SID: " + pmsCore.getUuid()).getBytes());
				out.write(CRLF.getBytes());
		...
		output.setHeader("Server", pmsCore.getServerName());

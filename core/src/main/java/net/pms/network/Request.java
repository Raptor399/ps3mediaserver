...
public class Request extends HTTPResource {
	...
	public interface Factory {
		public Request create(@Assisted("method") String method,
	...
	public void answer(OutputStream output,
			StartStopListenerDelegate startStopListenerDelegate)
			throws IOException {
		this.output = output;

		long CLoverride = -2; // 0 and above are valid Content-Length values, -1
								// means omit
		if (lowRange != 0 || highRange != 0) {
			...
			if (soapaction != null
					&& soapaction
							.indexOf("ContentDirectory:1#X_GetFeatureList") > -1) {
		...
		// Samsung 2012 TVs have a problematic preceding slash that needs to be
		// removed.
		...
		if ((method.equals("GET") || method.equals("HEAD"))
			...
			List<DLNAResource> files = pmsCore.getRootFolder(mediaRenderer)
					.getDLNAResources(id, false, 0, 0, mediaRenderer);
			...
			if (files.size() == 1) {
				...
				if (fileName.startsWith("thumbnail0000")) {
					...
					output(output,
							"Content-Type: " + dlna.getThumbnailContentType());
					...
					output(output, "Expires: " + getFUTUREDATE() + " GMT");
					List<DLNAMediaSubtitle> subs = dlna.getMedia()
							.getSubtitleTracksList();

					if (subs != null && !subs.isEmpty()) {
						// TODO: maybe loop subs to get the requested subtitle
						// type instead of using the first one
					...
					inputStream = dlna.getInputStream(Range.create(lowRange,
							highRange, timeseek, timeRangeEnd), mediaRenderer);
					if (inputStream == null) {
						// No inputStream indicates that transcoding / remuxing
						// probably crashed.
						LOGGER.error("There is no inputstream to return for "
								+ name);
					} else {
						startStopListenerDelegate.start(dlna);
						output(output,
								"Content-Type: "
										+ getRendererMimeType(dlna.mimeType(),
												mediaRenderer));

						// Some renderers (like Samsung devices) allow a custom
						// header for a subtitle URL
						String subtitleHttpHeader = mediaRenderer
								.getSubtitleHttpHeader();

						if (subtitleHttpHeader != null
								&& !"".equals(subtitleHttpHeader)) {
							// Device allows a custom subtitle HTTP header;
							// construct it
							List<DLNAMediaSubtitle> subs = dlna.getMedia()
									.getSubtitleTracksList();
							...
							if (subs != null && !subs.isEmpty()) {
								DLNAMediaSubtitle sub = subs.get(0);
								String subtitleUrl;
								String subExtension = sub.getType()
										.getExtension();
								if (isNotBlank(subExtension)) {
									subtitleUrl = "http://"
											+ pmsCore.getServer().getHost()
											+ ':'
											+ pmsCore.getServer().getPort()
											+ "/get/" + id + "/subtitle0000."
											+ subExtension;
								} else {
									subtitleUrl = "http://"
								...
								output(output, subtitleHttpHeader + ": "
										+ subtitleUrl);
							}
						}
						...
						// Response generation:
						// We use -1 for arithmetic convenience but don't send
						// it as a value.
						// If Content-Length < 0 we omit it, for Content-Range
						// we use '*' to signify unspecified.

						boolean chunked = mediaRenderer.isChunkedTransfer();

						// Determine the total size. Note: when transcoding the
						// length is
						// not known in advance, so DLNAMediaInfo.TRANS_SIZE
						...
						if (requested != 0) {
							// Determine the range (i.e. smaller of known or
							// requested bytes)
							long bytes = remaining > -1 ? remaining
							...
							// Calculate the corresponding highRange (this is
							// usually redundant).
							highRange = lowRange + bytes - (bytes > 0 ? 1 : 0);

							LOGGER.trace((chunked ? "Using chunked response. "
									: "") + "Sending " + bytes + " bytes.");
							...
							// Content-Length refers to the current chunk size
							// here, though in chunked
							// mode if the request is open-ended and totalsize
							// is unknown we omit it.
							...
							// Content-Length refers to the total remaining size
							// of the stream here.
				...
				&& (argument.toLowerCase().endsWith(".png")
						|| argument.toLowerCase().endsWith(".jpg") || argument
			...
			inputStream = getResourceInputStream(argument);
				...
				&& (argument.equals("description/fetch") || argument
						.endsWith("1.0.xml"))) {
			...
			output(output, "Connection: keep-alive");
			inputStream = getResourceInputStream((argument
					.equals("description/fetch") ? "PMS.xml" : argument));

			if (argument.equals("description/fetch")) {
				byte b[] = new byte[inputStream.available()];
				inputStream.read(b);
				String s = new String(b);
				s = s.replace("[uuid]", pmsCore.getUuid());// .substring(0,
															// pmsCore.usn().length()-2));
				...
				if (xbox) {
					...
					s = s.replace("PS3 Media Server", "PS3 Media Server ["
							+ profileName + "] : Windows Media Connect");
					s = s.replace("<modelName>PMS</modelName>",
				...
				&& (argument.contains("MS_MediaReceiverRegistrar_control") || argument
						.contains("mrr/control"))) {
			...
			response.append(CRLF);
				...
				&& argument.endsWith("upnp/control/connection_manager")) {
			output(output, CONTENT_TYPE_UTF8);
			...
			output(output, "SID: " + pmsCore.getUuid());
			output(output, "Server: " + pmsCore.getServerName());
			...
			try {
				...
				output(out, "SID: " + pmsCore.getUuid());
				output(out, "SEQ: " + 0);
				...
				LOGGER.debug(
						"Cannot parse address and port from soap action \""
								+ soapaction + "\"", ex);
			}

			if (argument.contains("connection_manager")) {
				response.append(HTTPXMLHelper
						.eventHeader("urn:schemas-upnp-org:service:ConnectionManager:1"));
				...
				response.append(HTTPXMLHelper
						.eventHeader("urn:schemas-upnp-org:service:ContentDirectory:1"));
				response.append(HTTPXMLHelper.eventProp("TransferIDs"));
				response.append(HTTPXMLHelper.eventProp("ContainerUpdateIDs"));
				response.append(HTTPXMLHelper.eventProp("SystemUpdateID", ""
						+ DLNAResource.getSystemUpdateId()));
				response.append(HTTPXMLHelper.EVENT_FOOTER);
			}
		} else if (method.equals("POST")
				&& argument.endsWith("upnp/control/content_directory")) {
			output(output, CONTENT_TYPE_UTF8);
			if (soapaction.indexOf("ContentDirectory:1#GetSystemUpdateID") > -1) {
				...
				response.append(CRLF);
					...
					.indexOf("ContentDirectory:1#GetSortCapabilities") > -1) {
				response.append(HTTPXMLHelper.XML_HEADER);
				response.append(CRLF);
				response.append(HTTPXMLHelper.SOAP_ENCODING_HEADER);
				response.append(CRLF);
				...
				response.append(CRLF);
					...
					.indexOf("ContentDirectory:1#X_GetFeatureList") > -1) { // Added
																			// for
																			// Samsung
																			// 2012
																			// TVs
				...
				response.append(CRLF);
					...
					.indexOf("ContentDirectory:1#GetSearchCapabilities") > -1) {
				response.append(HTTPXMLHelper.XML_HEADER);
				response.append(CRLF);
				response.append(HTTPXMLHelper.SOAP_ENCODING_HEADER);
				response.append(CRLF);
				...
				objectID = getEnclosingValue(content, "<ObjectID>",
						"</ObjectID>");
				String containerID = null;
				if ((objectID == null || objectID.length() == 0) && xbox) {
					containerID = getEnclosingValue(content, "<ContainerID>",
							"</ContainerID>");
				...
				Object sI = getEnclosingValue(content, "<StartingIndex>",
						"</StartingIndex>");
				Object rC = getEnclosingValue(content, "<RequestedCount>",
						"</RequestedCount>");
				browseFlag = getEnclosingValue(content, "<BrowseFlag>",
						"</BrowseFlag>");
				if (sI != null) {
				...
				if (xbox && configuration.getUseCache()
						&& pmsCore.getLibrary() != null && containerID != null) {
					if (containerID.equals("7")
							&& pmsCore.getLibrary().getAlbumFolder() != null) {
						objectID = pmsCore.getLibrary().getAlbumFolder()
								.getResourceId();
					} else if (containerID.equals("6")
							&& pmsCore.getLibrary().getArtistFolder() != null) {
						objectID = pmsCore.getLibrary().getArtistFolder()
								.getResourceId();
					} else if (containerID.equals("5")
							&& pmsCore.getLibrary().getGenreFolder() != null) {
						objectID = pmsCore.getLibrary().getGenreFolder()
								.getResourceId();
					} else if (containerID.equals("F")
							&& pmsCore.getLibrary().getPlaylistFolder() != null) {
						objectID = pmsCore.getLibrary().getPlaylistFolder()
								.getResourceId();
					} else if (containerID.equals("4")
							&& pmsCore.getLibrary().getAllFolder() != null) {
						objectID = pmsCore.getLibrary().getAllFolder()
								.getResourceId();
					} else if (containerID.equals("1")) {
						String artist = getEnclosingValue(content,
								"upnp:artist = &quot;", "&quot;)");
						if (artist != null) {
							objectID = pmsCore.getLibrary().getArtistFolder()
									.getResourceId();
				...
				List<DLNAResource> files = pmsCore
						.getRootFolder(mediaRenderer)
						.getDLNAResources(
								objectID,
								browseFlag != null
										&& browseFlag
												.equals("BrowseDirectChildren"),
								startingIndex, requestCount, mediaRenderer);
				...
				if (files != null) {
					for (DLNAResource uf : files) {
						...
						if (uf.isCompatible(mediaRenderer)
								&& (uf.getPlayer() == null || uf.getPlayer()
										.isPlayerCompatible(mediaRenderer))) {
				...
				response.append("<NumberReturned>").append(filessize - minus)
						.append("</NumberReturned>");
				response.append(CRLF);
				...
				if (browseFlag != null
					...
					// with the new parser, files are parsed and analyzed
					// *before*
					// creating the DLNA tree, every 10 items (the ps3 asks 10
					// by 10),
					// so we do not know exactly the total number of items in
					// the DLNA folder to send
					// (regular files, plus the #transcode folder, maybe the
					// #imdb one, also files can be
					// invalidated and hidden if format is broken or encrypted,
					// etc.).
					// let's send a fake total size to force the renderer to ask
					// following items
					int totalCount = startingIndex + requestCount + 1; // returns
																		// 11
																		// when
																		// 10
																		// asked
					if (filessize - minus <= 0) // if no more elements, send
												// startingIndex
					{
						totalCount = startingIndex;
					}
					response.append("<TotalMatches>").append(totalCount)
							.append("</TotalMatches>");
				} else if (browseFlag != null
						&& browseFlag.equals("BrowseDirectChildren"))
					response.append("<TotalMatches>")
							.append(((parentFolder != null) ? parentFolder
									.childrenNumber() : filessize) - minus)
							.append("</TotalMatches>");
				else
					// from upnp spec: If BrowseMetadata is specified in the
					// BrowseFlags then TotalMatches = 1
		...
		if (response.length() > 0) {
			...
			if (CLoverride > -2) {
				// Content-Length override has been set, send or omit as
				// appropriate
				if (CLoverride > -1 && CLoverride != DLNAMediaInfo.TRANS_SIZE) {
					// Since PS3 firmware 2.50, it is wiser not to send an
					// arbitrary Content-Length,
					// as the PS3 will display a network error and request the
					// last seconds of the
					// transcoded video. Better to send no Content-Length at
					// all.
			...
			if (timeseek > 0 && dlna != null) {
				String timeseekValue = DLNAMediaInfo
						.getDurationString(timeseek);
				String timetotalValue = dlna.getMedia().getDurationString();
				output(output, "TimeSeekRange.dlna.org: npt=" + timeseekValue
						+ "-" + timetotalValue + "/" + timetotalValue);
				output(output, "X-Seek-Range: npt=" + timeseekValue + "-"
						+ timetotalValue + "/" + timetotalValue);
			}
			...
			if (lowRange != DLNAMediaInfo.ENDFILE_POS && !method.equals("HEAD")) {
				sendB = sendBytes(inputStream); // , ((lowRange > 0 && highRange
												// > 0)?(highRange-lowRange):-1)
			}
	...
	private int sendBytes(InputStream fis) throws IOException {
		...
		try {
			...
			LOGGER.trace("Sending stream with premature end: " + sendBytes
					+ " bytes of " + argument + ". Reason: " + e.getMessage());
		} finally {
			fis.close();
		}
		return sendBytes;
	...
	private String getEnclosingValue(String content, String leftTag,
			String rightTag) {
		String result = null;
		...
		if (leftTagPos > -1 && rightTagPos > leftTagPos) {
			result = content.substring(leftTagPos + leftTag.length(),
					rightTagPos);
		}
		return result;
	}
